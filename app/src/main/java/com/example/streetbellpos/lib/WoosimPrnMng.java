package com.example.streetbellpos.lib;
...
import com.example.streetbellpos.utils.PrefMng;
...
public class WoosimPrnMng {
    ...
    public void printStr(String string, int mCharsize, int mJustification, boolean isbold) {
        printStr(string, isbold, false, mCharsize, mJustification);
    }
    ...
    protected void simplePrintStr(boolean mEmphasis, boolean mUnderline,
        ...
        byte[] cmd_CodeTable = WoosimCmd.setCodeTable(WoosimCmd.MCU_RX, PrefMng.getWoosimCodeTbl(), WoosimCmd.FONT_MEDIUM);
        byte[] cmd_default_CT = WoosimCmd.setCodeTable(WoosimCmd.MCU_RX, WoosimCmd.CT_CP437, WoosimCmd.FONT_MEDIUM);
     ...
     * @param maxWidth
     */
    public void printBitmap(Bitmap pic, int maxWidth) {
        if (pic == null) {
        ...
        int x = (maxWidth - w) / 2;
        byte[] data = WoosimImage.fastPrintBitmap(x, 5, w, pic.getHeight(), pic);
        pic.recycle();
